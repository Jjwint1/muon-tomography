...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    G4double fracMass;
    G4String symbol;
    G4Material* rho_1_5 = new G4Material("rho_1.5", density=1.5*g/cm3, ncomp=10);
    ...
    rho_1_5 -> AddElement(elH, fracMass=0.01*perCent);
    rho_1_5 -> AddElement(elC, fracMass=0.001*perCent);
    rho_1_5 -> AddElement(elO, fracMass=0.529107*perCent);
    rho_1_5 -> AddElement(elNa, fracMass=0.016*perCent);
    rho_1_5 -> AddElement(elMg, fracMass=0.002*perCent);
    rho_1_5 -> AddElement(elAl, fracMass=0.033872*perCent);
    rho_1_5 -> AddElement(elSi, fracMass=0.337021*perCent);
    rho_1_5 -> AddElement(elK, fracMass=0.013*perCent);
    rho_1_5 -> AddElement(elCa, fracMass=0.044*perCent);
    rho_1_5 -> AddElement(elFe, fracMass=0.014*perCent);
    ...
    //rho_1_5Cube
    G4VSolid* rho_1_5CubeSolid = new G4Box("rho_1_5CubeBox", 0.5*m, 0.5*m, 5*m);
    G4LogicalVolume* rho_1_5CubeLogical = new G4LogicalVolume(rho_1_5CubeSolid, rho_1_5, "rho_1_5CubeLogical");
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), rho_1_5CubeLogical, "rho_1_5CubePhysical", worldLogical, false, 0, checkOverlaps);
    
    //detectingSheets
    G4VSolid* detectingSheetSolid = new G4Box("detectingSheetBox", 0.5*m, 0.5*m, 0.00001*cm);
    
    for(G4int i = 0; i < 101; i++) {
        mDetectingSheets.push_back(new G4LogicalVolume(detectingSheetSolid, rho_1_5, "detectingSheetLogical"));
        G4double z = (-500.*cm + i * 10.*cm);
        new G4PVPlacement(0, G4ThreeVector(0., 0., z), mDetectingSheets[i], "detectingSheetPhysical", rho_1_5CubeLogical, false, 0, false);
    }
