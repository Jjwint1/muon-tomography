...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    G4double fracMass;
    G4String symbol;
    G4Material* rho_3_6 = new G4Material("rho_3.6", density=3.6*g/cm3, ncomp=10);
    ...
    rho_3_6 -> AddElement(elH, fracMass=0.01*perCent);
    rho_3_6 -> AddElement(elC, fracMass=0.001*perCent);
    rho_3_6 -> AddElement(elO, fracMass=0.529107*perCent);
    rho_3_6 -> AddElement(elNa, fracMass=0.016*perCent);
    rho_3_6 -> AddElement(elMg, fracMass=0.002*perCent);
    rho_3_6 -> AddElement(elAl, fracMass=0.033872*perCent);
    rho_3_6 -> AddElement(elSi, fracMass=0.337021*perCent);
    rho_3_6 -> AddElement(elK, fracMass=0.013*perCent);
    rho_3_6 -> AddElement(elCa, fracMass=0.044*perCent);
    rho_3_6 -> AddElement(elFe, fracMass=0.014*perCent);
    ...
    //rho_3_6Cube
    G4VSolid* rho_3_6CubeSolid = new G4Box("rho_3_6CubeBox", 0.5*m, 0.5*m, 5*m);
    G4LogicalVolume* rho_3_6CubeLogical = new G4LogicalVolume(rho_3_6CubeSolid, rho_3_6, "rho_3_6CubeLogical");
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), rho_3_6CubeLogical, "rho_3_6CubePhysical", worldLogical, false, 0, checkOverlaps);
    
    //detectingSheets
    G4VSolid* detectingSheetSolid = new G4Box("detectingSheetBox", 0.5*m, 0.5*m, 0.00001*cm);
    
    for(G4int i = 0; i < 101; i++) {
        mDetectingSheets.push_back(new G4LogicalVolume(detectingSheetSolid, rho_3_6, "detectingSheetLogical"));
        G4double z = (-500.*cm + i * 10.*cm);
        new G4PVPlacement(0, G4ThreeVector(0., 0., z), mDetectingSheets[i], "detectingSheetPhysical", rho_3_6CubeLogical, false, 0, false);
    }
